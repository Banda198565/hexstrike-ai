#!/usr/bin/env bash
# Connect cloud DeepSeek R1 (OpenRouter / DeepSeek official) for HexStrike Reasoning Agent.
#
# Usage:
#   export OPENROUTER_API_KEY=sk-or-...
#   bash scripts/connect-cloud-r1-orchestrator.sh
#   bash scripts/connect-cloud-r1-orchestrator.sh --status
#   bash scripts/connect-cloud-r1-orchestrator.sh --handshake
#   bash scripts/connect-cloud-r1-orchestrator.sh --plan config/reasoning-protocol.example.json
set -euo pipefail
...
for arg in "$@"; do
  case "$arg" in
    ...
    --plan) ;; # value handled by next arg in loop below - skip
    -h|--help)
      sed -n '2,14p' "$0"
      exit 0
      ;;
...
print(f"[cloud-r1] provider={cfg.provider} base={cfg.base_url} model={cfg.model}")
print(f"[cloud-r1] authenticated={bool(cfg.api_key)}")
...
# connect: upsert R1_* block into .env
if not env_path.is_file() and (root / ".env.example").is_file():
    ...
    print(f"[cloud-r1] seeded {env_path} from .env.example")
...
for k, v in updates.items():
    if k not in keys_seen:
        out.append(f"{k}={v}")
env_path.write_text("\n".join(out) + "\n", encoding="utf-8")
print(f"[cloud-r1] wrote R1_* block -> {env_path}")
